...
<div class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-sagra-line/80">
    ...
    <div class="my-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div class="rounded-lg bg-sagra-softer px-4 py-3">
            ...
            @else
                <p class="mt-2 text-sm text-sagra-muted">Scegli una serata B dal selettore sopra.</p>
            @endif
        </div>
    </div>
    ...
    @if ($a && $b)
        <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div class="rounded-md ring-1 ring-sagra-line px-3 py-2 text-sm">
            ...
            <tbody class="divide-y divide-sagra-line">
                @forelse ($piatti as $p)
                    <tr>
                        ...
                        <td class="px-3 py-2 tabular-nums">{{ $p['qta_a'] }}</td>
                        <td class="px-3 py-2 tabular-nums">{{ $p['qta_b'] }}</td>
                        <td class="px-3 py-2 tabular-nums font-semibold">{{ $p['delta'] > 0 ? '+'.$p['delta'] : $p['delta'] }}</td>
                    </tr>
                @empty
