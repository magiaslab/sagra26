<div class="report-sheet rounded-lg bg-white p-5 shadow-sm ring-1 ring-sagra-line/80">
    <div class="flex flex-wrap items-baseline justify-between gap-2">
        <div>
            ...
            <div class="report-meta text-xs text-sagra-muted">
                Report {{ $dati['titolo'] ?? \App\Models\MenuItem::etichettaArea($dati['area'] ?? null) }}
        ...
        <div class="flex flex-wrap gap-2 report-meta">
            ...
            <span class="text-xs font-medium text-sagra-muted">Cumulato serate {{ $dati['copertiCum'] }}</span>
        </div>
    </div>

    @php
        $sezioni = $dati['sezioni'] ?? [];
        if ($sezioni === [] && ! empty($dati['categorie'])) {
            $sezioni = [[
                'area' => $dati['area'] ?? '',
                'label' => null,
        ...
        $mostraIntestazioneArea = ($dati['mode'] ?? '') === 'cumulativo' && count($sezioni) > 1;
    @endphp

    @forelse ($sezioni as $sez)
        @if ($mostraIntestazioneArea)
            <div class="mb-2 mt-5 border-b-2 border-sagra-ink pb-1 text-base font-bold uppercase tracking-wide text-sagra-ink">
                {{ $sez['label'] }}
            </div>
        @endif

        @forelse ($sez['categorie'] as $cat)
            <h3 class="mb-2 {{ $mostraIntestazioneArea ? 'mt-3' : 'mt-4' }} border-b border-sagra-line pb-1 text-sm font-semibold text-sagra-ink">
                {{ $cat->nome }}
            </h3>
                ...
                <table class="min-w-full divide-y divide-sagra-line text-sm">
                    <thead>
                        <tr class="bg-sagra-softer">
                            ...
                            <th class="w-28 px-3 py-2 text-right font-semibold text-sagra-ink">Cumulato</th>
        ...
        @empty
            <p class="text-sm text-sagra-muted">Nessuna voce in {{ $sez['label'] ?? 'questo reparto' }}.</p>
        @endforelse
    @empty
        <p class="mt-4 text-sm text-sagra-muted">Nessuna voce di produzione per questo report.</p>
    @endforelse
</div>
