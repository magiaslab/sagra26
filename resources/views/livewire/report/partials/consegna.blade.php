    ...
    @else
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <div class="report-meta text-sm text-sagra-ink">{{ $impostazioni->intestazione_nome }} {{ $impostazioni->intestazione_anno }}</div>
                <div class="report-meta text-sm text-sagra-muted">{{ $dati['punto']->nome }}</div>
                @if (! $serata->isAperta())
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-sagra-line text-sm">
                <tbody class="divide-y divide-sagra-line">
                    <tr><td class="px-3 py-2">Incasso totale atteso</td><td class="px-3 py-2 text-right tabular-nums">{{ number_format($ric['atteso_totale'], 2, ',', '.') }} €</td></tr>
                    <tr><td class="px-3 py-2">Atteso contante</td><td class="px-3 py-2 text-right tabular-nums">{{ number_format($ric['atteso_contante'], 2, ',', '.') }} €</td></tr>
                    <tr><td class="px-3 py-2">Atteso POS</td><td class="px-3 py-2 text-right tabular-nums">{{ number_format($ric['atteso_pos'], 2, ',', '.') }} €</td></tr>
                    <tr><td class="px-3 py-2">Fondo iniziale</td><td class="px-3 py-2 text-right tabular-nums">{{ number_format($c->fondo_iniziale, 2, ',', '.') }} €</td></tr>
                    <tr><td class="px-3 py-2">Contante contato</td><td class="px-3 py-2 text-right tabular-nums">{{ number_format($c->contante_contato, 2, ',', '.') }} €</td></tr>
            ...
            <table class="min-w-full divide-y divide-sagra-line text-sm">
                <thead>
                    <tr class="bg-sagra-softer">
                        ...
                        <th class="px-3 py-2 text-right font-semibold text-sagra-ink">N°</th>
                        <th class="px-3 py-2 text-right font-semibold text-sagra-ink">Importo</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-sagra-line">
                @php $pezziStampati = 0; @endphp
                @foreach ($dati['tagli'] as $campo => $valore)
                    @if ((int) $c->{$campo} > 0)
                        @php $pezziStampati++; @endphp
                        <tr>
                            <td class="px-3 py-2">{{ number_format($valore, $valore < 1 ? 2 : 0, ',', '.') }} €</td>
                            <td class="px-3 py-2 text-right tabular-nums">{{ $c->{$campo} }}</td>
                            <td class="px-3 py-2 text-right tabular-nums">{{ number_format($c->{$campo} * $valore, 2, ',', '.') }} €</td>
                        </tr>
                    @endif
                @endforeach
                @if ($pezziStampati === 0)
                    <tr><td colspan="3" class="px-3 py-2 text-sagra-muted">Nessun pezzo contato.</td></tr>
                @endif
                </tbody>
            </table>
        </div>

        <div class="report-firma mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
