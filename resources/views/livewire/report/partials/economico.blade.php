        ...
        <table class="min-w-full divide-y divide-sagra-line text-sm">
            ...
            @foreach ($dati['righe'] as $r)
                <tr>
                    ...
                    <td class="px-3 py-2">{{ number_format($r['contante'], 2, ',', '.') }} €</td>
                    <td class="px-3 py-2">{{ number_format($r['pos'], 2, ',', '.') }} €</td>
                    <td class="px-3 py-2">{{ number_format($r['totale'], 2, ',', '.') }} €</td>
                </tr>
            @endforeach
            <tr class="bg-sagra-softer font-semibold">
                <td class="px-3 py-2">TOTALE</td>
                <td class="px-3 py-2">{{ number_format($dati['tot_contante'], 2, ',', '.') }} €</td>
                <td class="px-3 py-2">{{ number_format($dati['tot_pos'], 2, ',', '.') }} €</td>
                <td class="px-3 py-2">{{ number_format($dati['totale'], 2, ',', '.') }} €</td>
            </tr>
            </tbody>
        </table>
    </div>
    <p class="text-sm text-sagra-ink">Ripartizione: contante {{ $dati['pct_contante'] }}% · POS {{ $dati['pct_pos'] }}%</p>
    <p class="text-sm text-sagra-ink"><strong>di cui Bar: {{ number_format($dati['di_cui_bar'], 2, ',', '.') }} €</strong></p>
</div>
